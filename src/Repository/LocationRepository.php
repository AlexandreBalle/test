...
class LocationRepository extends ServiceEntityRepository
{
    ...
    public function getLocationPast($idUser){ //date de fin déjà passé
        ...
        $qb
            ->select('l.startDate, l.endDate, a.city, a.address, a.price, v.brand, v.model, v.matriculation, DATE_DIFF(l.endDate, l.startDate) AS dateDiff, l.returned_at')
    ...
    public function getLocationFutur($idUser){ //date de début dans le futur
        ...
        $qb = $this->createQueryBuilder('l');
        $qb
            ->select('l.startDate, l.endDate, a.city, a.address, a.price, v.brand, v.model, v.matriculation, DATE_DIFF(l.endDate, l.startDate) AS dateDiff')
            ->innerJoin(Announce::class, 'a', Join::WITH, 'a.id = l.announce')
            ->innerJoin(Vehicle::class, 'v', Join::WITH, 'v.id = a.vehicle')
